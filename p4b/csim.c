...
// global var to track next val for LRU
int lru_cnt = 0;
 ...
 */                    
void init_cache() {          
    // initialize and determine the value of S (S = 2^s)
    S = 1;
    for(int i = 0; i < s; i++){
        S *= 2;
    }
    // allocate S sets in memory
    ...
    for(int i = 0; i < S; i++){
        // allocates E lines in current i set
        *(cache + i) = (cache_set_t) malloc(sizeof(cache_line_t) * E);
	if (*(cache +i) == NULL) {
	    printf("Unable to allocate memory.\n");
	    exit(1);
	}
	// sets valid bits, tag bits, and lru count to 0
	for(int j = 0; j < E; j ++){
            (*(*(cache + i) + j)).valid = 0;
	    (*(*(cache + i) + j)).tag = 0;
	    (*(*(cache + i) + j)).lru = 0;
...
void access_data(mem_addr_t addr) {      
    // use right and left shift to get set and tag
    ...
    // if program reaches this far, it has not returned and so there was a miss
    miss_cnt++;
    ...
    // if program reacehs this far, eviction is needed
    evict_cnt++;

    // change the line
    int line_num = 0; // line to be changed
    for(int i = 0; i < E; i++){
        if((*(*(cache + set) + i)).lru < (*(*(cache + set) + line_num)).lru){
	    line_num = i;
	}
    }
    // update line that needs to be modified
    (*(*(cache + set) + line_num)).lru = lru_cnt;
    (*(*(cache + set) + line_num)).tag = tag;
}
